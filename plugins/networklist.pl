...
my %config = (hive          => "Software",
              hivemask      => 0x08, #Software hive
              class         => 0,
              type          => "Reg",
              osmask        => 52, #Vista,Win7/2008R2,Win8
              output        => "report",
              category      => "Networking",
              hasShortDescr => 1,
              hasDescr      => 0,
...
sub pluginmain {
	...
	::logMsg("networklist v.".$VERSION);
	::rptMsg("-" x 60);
	::rptMsg("networklist v.".$VERSION);
		...
		if (scalar(@sk) > 0) {
			foreach my $s (@sk) {
				...
				eval {
					...
					$nl{$name}{DateCreated} = ::convertSystemTime($create) if (length($create) == 16);
					my $conn   = $s->get_value("DateLastConnected")->get_data();
					$nl{$name}{DateLastConnected} = ::convertSystemTime($conn) if (length($conn) == 16);
